...
| [`pr-context-cache`](../skills/pr-context-cache.md) | Raana | `/maigo:review` step 1 | 把 PR title/body/diff/CI status/linked issues cache 到 `review-rubric-<id>.md`，re-review 時跳過重抓 |
...
| [`model-dispatch`](../skills/model-dispatch.md) | orchestrator | Maigo command 執行角色前，所有宿主 | 明確選取的角色模型 profile、宿主能力判斷、單一模型 fallback 與兩輪重試預算 |
