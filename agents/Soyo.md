...
**4. 蒐集 triggered skills**：對所有 `type: convention` entry 讀 frontmatter `triggers`（可能不存在或空）。對每個 `<name>`：
...
**只有 `type: convention` 的 entry 適用 triggers**——其他 type 的 `triggers` 欄位無聲忽略。
