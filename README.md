...
Maigo 是一套以 **MyGO!!!!!** 五位團員為人設的多 agent 通用開發工作流（Claude Code / Codex plugin）。
...
| 需要 | 版本 | 用途 |
|------|------|------|
| Claude Code CLI 或 Codex CLI | latest | Plugin host |
...
```bash
codex plugin marketplace add Lee-W/maigo
codex plugin add maigo@maigo
```

Codex 目前不會把 Claude Code 的 `commands/` 轉成原生 slash command。安裝後請使用
`maigo:go <task>`、`maigo:review <source>`、`maigo:retro` 這類不以 `/` 開頭的 pseudo-command；
Maigo 的 `command-router` skill 會載入對應 `commands/*.md` 作為唯一行為來源。
