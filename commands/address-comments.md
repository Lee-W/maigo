...
把三種意見來源都抓齊（orchestrator 直接跑 `gh`）：inline review threads（含 `isResolved`）、
review 摘要、conversation comments。三段查詢指令逐字稿（GraphQL query、`<owner>`/`<repo>`
取法、`--json reviews` 缺 `url` 的補丁）見
`skills/github-reply-draft/references/comment-fetch-and-triage.md`「抓取」。
...
**驗證現狀，不要照抄意見原文當 TODO**：抓齊三種來源後，逐條對照該 comment 指向的 `file:line`
在**目前 HEAD** 的實際內容——force-push / 後續 fixup 可能已經部分或全部處理掉某條意見，thread
標 `unresolved` 不代表現狀仍然如此。每條判進三桶之一：`已處理`（現狀已符合意見字面）、
`部分處理`（原本較大的訴求只剩一部分還沒做）、`仍有效`。**「已處理」只確認字面訴求，不代表行為
正確**——若現狀讓人懷疑行為本身有問題（即使字面訴求技術上已滿足），照實列出讓使用者選要不要
順便處理，不要自動歸類成「已處理」了事。若某條意見是 GitHub `suggestion` block，把建議文字對照
...
（`已處理`/`部分處理`/`仍有效`）+ 一句 rationale，不要先丟一行式清單、等使用者一條條追問才展開。
預設**摺疊已解決 / outdated 的 thread**（只印一行「另有 N 條已解決，預設略過」），其餘逐條列：
...
若某條有多種可行做法（例如 `normpath` vs `realpath` vs `abspath`），直接在清單裡附一個精簡的
trade-off 對照，不要等使用者問了才展開。已處理的條目在清單行首直接寫「已處理 — 指向
`file:line`」，讓使用者可以直接選「回覆就好」而不必重讀一次程式碼。
...
**預設盡量走 `/maigo:quick`**——多數 review 意見是局部修正；不確定 quick 還是 go → 偏
`/maigo:go`（多一輪探索 + 完整 9 項 review，往上靠較安全）。

**修法有「最小正確」與「順便加強」兩層時，先估兩種大小，帶最小版本讓使用者選**：若某條意見的
修法可以拆成「解掉核心問題的最小版」與「還順便補強 ergonomics/robustness 的完整版」，work item
的 rationale 要誠實列出兩種大小的估計行數，預設帶最小正確版本進路由計畫；完整版若值得做，另外
標成後續 follow-up（有 tracking issue 就附連結，沒有就在 rationale 寫一句取捨），不要預設把完整版
塞進當前 work item 讓 diff 看起來「比較完整」。
