...
- **Review 摘要**（APPROVE / REQUEST_CHANGES / COMMENT + 整體留言）：
  ...
  > **注意**：`gh pr view --json reviews` 回傳的 review 物件**不含 `url`**。
  > review 的 permalink 需另行用 GraphQL 查詢，或由 review `id`（`PRR_…`）換算成
  > anchor 形式 `<PR-url>#pullrequestreview-<numeric-databaseId>`。
  > 建議補一道 GraphQL 抓 `url`：
  >
  > ```bash
  > gh api graphql -f query='
  ...
  > ```
  >
  > 此查詢傳回的 `url` 格式為 `<PR-url>#pullrequestreview-<id>`，可直接用於步驟 4 triage 檔與步驟 6 Finale。
