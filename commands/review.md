...
**先套 [`skills/pr-context-cache`](https://github.com/Lee-W/maigo/blob/main/skills/pr-context-cache/SKILL.md)**：第一次 fetch 後 cache 到 `.maigo/review-rubric.md` 開頭的 `<!-- pr-context-cache:start v1 -->` 段。後續 re-review 偵測同一段且 diff sha 未變 → 跳過 `gh pr view / gh pr diff / gh pr checks` 重抓。
