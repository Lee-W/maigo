...
**批次處理同一輪 review comment 時，事實會用兩種方式過期。** comment 裡的行號、次數、
對行為的敘述都是對 reviewer 當時看到的版本寫的，動手前一律重新實測，不要照抄：

1. **時間漂移**——branch 動過（rebase、或同批前面幾條的修法），行號與計數就不準；
2. **同批 comment 互相作廢**——收第 N 條時做的改動，可能讓第 M 條的敘述變成不實，而
   兩條是同一輪要收的。

第二種比單純行號錯更糟：照著已被作廢的句子去改，等於把一個新的不實敘述寫進去，而且
它看起來有 reviewer 背書，測試與 docs build 都不會抓到。**每收完一條，回頭掃一次同批
還沒收的**——這條的修法有沒有讓別條的前提不成立？特別是改了行為判準（誰會被轉發、
什麼時候 raise）的那種。發現某條被作廢，**不要照抄它的句子**，以現況重新推導，並在
回覆裡明說哪一句因為什麼修法而不再準確——reviewer 會想知道。reviewer 舉的例子本身也
可能是錯的（不只是過期），前提是他的宣稱，一樣要一手查證。
