...
- **`📄 產物路徑`**（optional）：指向這項的本地產物（review-<n>.md / triage 筆記等），相對
  `.maigo/` 的路徑。沒產物就省略。served 頁面上這段維持 inline code（不是超連結——
  行文法不塞 markdown link，見 §6）；該檔案本身在 `docs_dir` 內同樣會被渲染成一頁，
  使用者從 mkdocs 側邊 nav 點得到
...
## 6. 閱讀層：`--serve`（mkdocs live reload）
...
1. **scaffold**：首跑在 `.maigo/_serve/` 生成最小 `mkdocs.yml` ＋ `board-style.css`
   （gitignored 工作區，不是真相層；已存在就不覆寫，保留使用者調整）。
2. **docs_dir 直指 `.maigo/`**：`board.md` 跟其他 `.md`（📄 連結的 review report 等產物）
   由同一個 server 直接渲染——不再有另外的 render 步驟，也不再另外產生一份閱讀層檔案。
   `📄 \`路徑\`` 這段行文法本身在 served 頁面維持 inline code（不是超連結；board.md
   不塞 markdown link 語法），但該檔案照樣在 `docs_dir` 裡被渲染成一頁，使用者從
   mkdocs 側邊 nav 點得到——跟 v1（pandoc 把 📄 一段轉成內文超連結）的 UX 不同，
   這是 v2 已知、可接受的行為差異。
3. **checkbox**：`mkdocs.yml` 帶 `pymdownx.tasklist`（`custom_checkbox: true`），
   served 頁面把 `- [ ]` 渲染成真正的 `<input type="checkbox">`（唯讀；勾選回寫
   一樣只能改真相層 `board.md` 本身，served 頁面不能寫回）。
4. **首頁**：`board.md` 不是 `index.md`——script 不額外造一份 index scaffold 去污染
   `.maigo/`，直接印出 board 頁網址（`http://<addr>/board/`）。
5. **啟動優先序**：cwd 專案 venv 已裝 mkdocs → `uv run mkdocs serve -f <config>`；
   沒有 → `uvx --from mkdocs --with pymdown-extensions mkdocs serve -f <config>`
   （zero repo 相依，maigo 在其他 repo 的 `.maigo/` 一樣可用）。
6. **live reload**：mkdocs 內建 watch，agent 寫 board.md 或使用者 nvim 存檔，served
   頁面幾秒內更新，不需要手動重跑任何 render 指令。
7. **退場門**：serve 引擎只是手段——`config` 極小、真相層是純 markdown，隨時可換掉
   ...
   渲染腳本已整檔退役，不再維護。
8. **只能前景執行**：`--serve` 要一直開著終端機、Ctrl-C 結束，不要背景 detach——
   父行程被 SIGTERM 終止時子行程會孤兒化佔 port。

## What this skill does NOT cover

- `/maigo:board` 的命令面（無參數刷新 / `<targets...>` / `--all` / `--serve` / `--learn` / `--drop`）——
